...
func TestExec(t *testing.T) {
	...
	for i, c := range []*execTestCase{
		...
		c(POPk).want().work(0),
		c(POPk).work(1).want().work(1, 0),
		c(POP2k).want().work(0, 0),
		c(POP2k).work(1).want().work(1, 0, 0),

		c(NIP).work(1, 2).want().work(2),
		c(NIP2).work(1, 2, 3, 4).want().work(3, 4),

		c(NIPk).work(1).want().work(0, 1),
		c(NIP2k).work(1, 2).want().work(0, 0, 1, 2),
		c(NIP2k).work(1, 2, 3, 4).want().work(1, 2, 0, 0, 3, 4),
		c(NIPk).want().
